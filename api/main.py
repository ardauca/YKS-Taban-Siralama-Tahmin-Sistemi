...
def _load_or_train_models():
    ...
    _lgb_low = lgb.LGBMRegressor(objective="quantile", alpha=0.080, **lgb_params).fit(X_train, y_train)
    _lgb_upp = lgb.LGBMRegressor(objective="quantile", alpha=0.920, **lgb_params).fit(X_train, y_train)
    ...
    _cb_low = CatBoostRegressor(loss_function="Quantile:alpha=0.080", iterations=300, learning_rate=0.04, depth=6, verbose=0, random_seed=42).fit(X_train, y_train)
    _cb_upp = CatBoostRegressor(loss_function="Quantile:alpha=0.920", iterations=300, learning_rate=0.04, depth=6, verbose=0, random_seed=42).fit(X_train, y_train)
